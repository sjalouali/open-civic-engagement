...
export class PropositionServiceOceUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.typeServiceService.queryPublic().subscribe((res: HttpResponse<ITypeServiceOce[]>) => (this.typeservices = res.body || []));
    // INIT form demande
  ...
  onSuggestion($event: any): void {
    ...
    this.editForm.patchValue({
      labelAdresse: $event.suggestion.name,
      locality: $event.suggestion.city,
      county: $event.suggestion.county,
