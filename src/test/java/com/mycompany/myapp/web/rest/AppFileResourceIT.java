...
import com.mycompany.myapp.service.AppFileQueryService;
...
public class AppFileResourceIT {
    ...
    @Test
    @Transactional
    public void getAllAppFilesByCommentIsEqualToSomething() throws Exception {
        // Initialize the database
        appFileRepository.saveAndFlush(appFile);
        Comment comment = CommentResourceIT.createEntity(em);
        em.persist(comment);
        em.flush();
        appFile.setComment(comment);
        appFileRepository.saveAndFlush(appFile);
        Long commentId = comment.getId();

        // Get all the appFileList where comment equals to commentId
        defaultAppFileShouldBeFound("commentId.equals=" + commentId);

        // Get all the appFileList where comment equals to commentId + 1
        defaultAppFileShouldNotBeFound("commentId.equals=" + (commentId + 1));
    }
